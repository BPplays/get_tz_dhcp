...
func TestSprintSingleTz(t *testing.T) {
	...
	t.Run("print single tz 125:", func(t *testing.T) {
		t.Parallel()
		strs := []string{}
		...
		log.Printf("print single tz %v: %v\n", rang, s)
		log.Printf("print single tz %v: total time: %v per func: %v\n", rang, time.Since(st), time.Since(st) / time.Duration(rang))
